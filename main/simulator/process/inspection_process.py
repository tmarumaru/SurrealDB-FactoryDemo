...
import util
from simulator.factory_models import ProductionLine, OperationType, ProductStatus, ProductType
...
class InspectionProcess(AWorkManager):
    ...
    def __init__(
        ...
        """
        super(InspectionProcess, self).__init__(
            env=env,
    ...
    def run(
        ...
        while True:
            ...
            # ある比率で良品／不良品に振り分ける
            product.status = ProductStatus.FINISHED_PRODUCT \
            ...
            if product.status == ProductStatus.FINISHED_PRODUCT:
                # 良品
                product.product_type = ProductType.PARTS if product.specification_id == 'SP01' \
                    else ProductType.PRODUCT
                yield self.out_storage.put(product)
                wm.add_production_history(ProductionHistory(
                    operation_type=OperationType.FINISHED_PRODUCT,
                    product=product,
                    status=product.status,
                    timestamp=self._get_current_datetime(),
                ))
